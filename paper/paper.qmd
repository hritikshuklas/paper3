...
of our fitted logistic model, as seen in @fig-model-summary:
...
model_summary_tbl <- modelsummary(
  list(
    "Support Biden" = model
  ),
  type="latex",
...
is about 51% as seen in @code-log-odd-intercept, which falls in line
with our observations in @fig-pres-pref-gender-generation.

```{r, caption="Calculating probability from the intercept log odds"}
#| label: code-log-odd-intercept
#| warning: false
